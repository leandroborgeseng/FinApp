...
import { ContasTab } from './screens-contas.jsx';
// screens-gestao.jsx — Gestão: Financiamentos · Investimentos · Recorrências · Contas
    ...
    <div style={{ paddingBottom: 18, marginBottom: 6, borderBottom: '1.5px solid #ECEEF4' }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${catGrad[0]},${catGrad[1]})`, borderRadius: 16, padding: '14px 16px', marginBottom: 10, boxShadow: '0 3px 14px rgba(0,0,0,0.18)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-inverse)', letterSpacing: '-0.5px' }}>{fmt(bal)}</div>
          </div>
          ...
          {[['Parcela', `${fmt(pmt)}/mês`], ['Término', fin.endYear], ['CET', `${fin.cet}%a.a.`], ['Restam', `${remN}×`]].map(([l, v], idx) => (
            <React.Fragment key={l}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-inverse)', marginTop: 1 }}>{v}</div>
              </div>
            </React.Fragment>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        <div style={{ flex: 1, background: '#FEF2F2', borderRadius: 10, padding: '9px 11px' }}>
          <div style={{ fontSize: 9, color: 'var(--text-muted)', marginBottom: 2 }}>Juros restantes</div>
        ...
        <div style={{ flex: 1, background: '#FFFBEB', borderRadius: 10, padding: '9px 11px' }}>
          <div style={{ fontSize: 9, color: 'var(--text-muted)', marginBottom: 2 }}>Total a pagar</div>
      ...
      <div style={{ display: 'flex', gap: 7, marginBottom: 10 }}>
        {[['prepay','Antecipar','#2563EB'],['settle','Quitar','#16A34A'],['saldo','Saldo','#F59E0B']].map(([mode, label, col]) => (
          <button key={mode} onClick={() => setSimMode(simMode === mode ? null : mode)} style={{
            ...
            border: `1.5px solid ${simMode === mode ? col : 'var(--border)'}`,
            background: simMode === mode ? col + '15' : 'var(--bg-card)',
            color: simMode === mode ? col : 'var(--text-muted)',
      ...
      {simMode === 'prepay' && (
        <div style={{ background: '#EFF6FF', borderRadius: 12, padding: '12px 14px', marginBottom: 4 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{fmt(pre.extraPaid)}</span>
          </div>
      ...
      {simMode === 'saldo' && (
        <div style={{ background: '#FFFBEB', borderRadius: 12, padding: '12px 14px', border: '1px solid #FDE68A', marginBottom: 4 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 6 }}>Novo saldo devedor (R$)</div>
          <input value={tempBal} onChange={e => setTempBal(e.target.value.replace(/\D/g, ''))}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #F59E0B', fontSize: 16, fontWeight: 700, color: '#F59E0B', background: 'var(--bg-card)', outline: 'none', fontFamily: 'DM Sans, system-ui', boxSizing: 'border-box' }} inputMode="numeric"/>
          <button onClick={() => { const v = Number(tempBal) || bal; setBal(v); onSaveBalance?.(fin.id, v); setSimMode(null); }} style={{ marginTop: 8, width: '100%', padding: '9px', borderRadius: 9, background: '#F59E0B', color: 'var(--text-inverse)', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, system-ui' }}>
            Confirmar saldo
          </button>
        </div>
      )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <Card style={{ padding: '12px 14px' }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Total em dívidas</div>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#DC2626' }}>{fmt(totalDebt, { short: true })}</div>
        </Card>
        <Card style={{ padding: '12px 14px' }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Parcelas/mês</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {['pf', 'pj'].map(entity => {
        ...
        return (
          <div key={entity}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>{label}</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{fmt(total, { short: true })} total</div>
            </div>
                    ...
                    <div onClick={() => setEditKey(isEditing ? null : key)}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.name}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{inv.ret}% a.a.{inv.monthly > 0 ? ` · +${fmt(inv.monthly, { short: true })}/mês` : ''}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{fmt(inv.value, { short: true })}</div>
                          <div style={{ fontSize: 9, color: 'var(--text-faint)' }}>editar</div>
                        </div>
                      </div>

                      {isEditing && (
                        <div style={{ margin: '10px 0 12px', padding: '12px', background: 'var(--bg-app)', borderRadius: 10 }}
                          onClick={e => e.stopPropagation()}>
                          {/* Projections */}
                          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                            {[[12, p12], [24, p24]].map(([mo, val]) => (
                              <div key={mo} style={{ flex: 1, background: accentBg, borderRadius: 8, padding: '8px 10px' }}>
                                <div style={{ fontSize: 9, color: 'var(--text-muted)' }}>em {mo} meses</div>
                          ...
                          {[
                            ...
                            <div key={field} style={{ marginBottom: 8 }}>
                              ...
                              <input defaultValue={inv[field]}
                                ...
                                style={{ width: '100%', padding: '9px 12px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', background: 'var(--bg-card)', outline: 'none', fontFamily: 'DM Sans, system-ui', boxSizing: 'border-box' }}
                                inputMode="decimal"/>
                            </div>
                          ))}
                          <button onClick={() => setEditKey(null)} style={{ width: '100%', padding: '9px', borderRadius: 9, border: 'none', background: 'var(--bg-toggle)', color: 'var(--text-muted)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, system-ui' }}>
                            Fechar
                          </button>
                        </div>
                      )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {groups.map(g => {
        ...
        return (
          <div key={g.type}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>{g.label}</div>
                    ...
                    <div onClick={() => setEditIdx(isEditing ? null : e._i)}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
                            ...
                            <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>dia {e.day} · {e.cat}</span>
                          </div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                      ...
                      {isEditing && (
                        <div style={{ margin: '10px 0 12px', padding: '12px', background: 'var(--bg-app)', borderRadius: 10 }}
                          ...
                          <button onClick={() => setEditIdx(null)} style={{ width: '100%', padding: '9px', borderRadius: 9, border: 'none', background: 'var(--bg-toggle)', color: 'var(--text-muted)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, system-ui' }}>Fechar</button>
                        </div>
                      )}
                    </div>
                  </div>
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg-app)', fontFamily: 'DM Sans, system-ui' }}>
      <div style={{ padding: 'var(--pad-top) var(--pad-x) var(--pad-bottom)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.2 }}>Gestão Financeira</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>Financiamentos · Investimentos · Contas · Recorrências</div>
          </div>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', background: 'var(--bg-toggle)', borderRadius: 14, padding: 3, gap: 2 }}>
          {[['financ','Financiamentos'],['invest','Investimentos'],['contas','Contas'],['recorr','Recorrências']].map(([k, l]) => (
            <button key={k} onClick={() => setTab(k)} style={{
              flex: 1, padding: '7px 4px', borderRadius: 11, border: 'none', cursor: 'pointer',
              background: tab === k ? 'var(--bg-card)' : 'transparent',
              color: tab === k ? '#1A1F36' : 'var(--text-muted)',
