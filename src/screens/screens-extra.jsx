...
function ComparativoMesesScreen({ onBack }) {
  ...
  const MonthSel = ({ idx, setIdx }) => (
    <select value={idx} onChange={e => setIdx(Number(e.target.value))} style={{ flex: 1, padding: '9px 10px', borderRadius: 12, border: '1.5px solid var(--border)', fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', background: 'var(--bg-card)', fontFamily: 'DM Sans, system-ui', cursor: 'pointer', outline: 'none' }}>
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg-app)', fontFamily: 'DM Sans, system-ui' }}>
      <div style={{ padding: 'var(--pad-top) var(--pad-x) var(--pad-bottom)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <BackBtn/>
          <div>
            <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--text-primary)' }}>Comparativo de Meses</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>Selecione dois meses para comparar</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <MonthSel idx={idxA} setIdx={setIdxA}/>
          <span style={{ fontSize: 14, color: 'var(--text-muted)', fontWeight: 700, flexShrink: 0 }}>vs.</span>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[ma, mb].map((m, i) => (
            <div key={i} style={{ flex: 1, background: 'linear-gradient(135deg,#1A1F36,#253056)', borderRadius: 16, padding: '14px 16px', boxShadow: '0 4px 16px rgba(26,31,54,0.18)' }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-inverse)', letterSpacing: '-0.5px', marginBottom: 10 }}>{fmt(m.saldo, {short:true})}</div>
        ...
        <Card style={{ padding: '4px 6px' }}>
          {rows.map((row, ri) => {
            ...
            return (
              <div key={row.key}>
                {ri > 0 && <div style={{ height: 1, background: 'var(--divider)', margin: '0 14px' }}/>}
                <div style={{ padding: '12px 14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[{lbl: ma.label, val: va, isDelta: false}, {lbl: mb.label, val: vb, isDelta: false}, {lbl: 'Delta', val: delta, isDelta: true}].map(({lbl, val, isDelta}) => (
                      <div key={lbl} style={{ flex: 1, background: isDelta ? (positive ? '#F0FDF4' : '#FEF2F2') : 'var(--bg-app)', borderRadius: 8, padding: '6px 8px' }}>
                        <div style={{ fontSize: 8, color: 'var(--text-muted)', marginBottom: 2 }}>{lbl}</div>
                        <div style={{ fontSize: 12, fontWeight: 700, color: isDelta ? (positive ? '#16A34A' : '#DC2626') : 'var(--text-primary)' }}>
  ...
  const numField = (label, val, setVal) => (
    <div style={{ marginBottom: 12 }}>
      ...
      <input
        ...
        style={{ width: '100%', padding: '11px 14px', borderRadius: 12, border: '1.5px solid var(--border)', fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', background: 'var(--bg-app)', outline: 'none', fontFamily: 'DM Sans, system-ui', boxSizing: 'border-box' }}
        inputMode="numeric"
      />
    </div>
  );
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg-app)', fontFamily: 'DM Sans, system-ui' }}>
      <div style={{ padding: 'var(--pad-top) var(--pad-x) var(--pad-bottom)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--text-primary)' }}>Calculadora de Rentabilidade</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>Simule qualquer investimento com IR</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {Object.entries(benchmarks).map(([k, b]) => (
            <button key={k} onClick={() => setBenchKey(k)} style={{ padding: '5px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', background: benchKey === k ? '#1A1F36' : 'var(--bg-card)', color: benchKey === k ? 'var(--bg-card)' : 'var(--text-muted)', fontSize: 11, fontWeight: 600, fontFamily: 'DM Sans, system-ui', boxShadow: benchKey === k ? '0 2px 8px rgba(26,31,54,0.2)' : 'none' }}>
        ...
        {benchKey === 'custom' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)' }}>% a.a.</span>
          </div>
        )}
        ...
        <Card style={{ padding: '14px 16px' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Prazo</span>
        ...
        <div style={{ background: 'linear-gradient(145deg,#1A1F36,#253056)', borderRadius: 22, padding: '20px 20px 18px', boxShadow: '0 6px 24px rgba(26,31,54,0.22)' }}>
          ...
          <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--text-inverse)', letterSpacing: '-0.5px', marginBottom: 14 }}>{fmt(netFV)}</div>
        ...
        <Card style={{ padding: '16px 14px 10px' }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 10 }}>Evolução do montante</div>
          <ChartBox height={110}>
            {(w, h) => <AreaChart data={chartData} width={w} height={h} color="#2563EB"/>}
          </ChartBox>
        </Card>

        {/* Tax detail */}
        <Card style={{ padding: '14px 16px' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 10 }}>Detalhamento</div>
          {[
...
export { ComparativoMesesScreen, CalculadoraRentabilidadeScreen };
